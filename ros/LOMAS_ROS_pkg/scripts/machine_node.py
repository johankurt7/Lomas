...
def loadParameters():
    ...
    rospy.loginfo("Machine param values:")
    rospy.loginfo(" * IsInSimMode: %s", IsInSimMode)
    rospy.loginfo(" * Port:        %s", port)
    rospy.loginfo(" * Path:        %s", path)
    rospy.loginfo(" * Interval:    %s", status.Interval)
    rospy.loginfo("")
...
def connectToMachine():
    ...
    if IsInSimMode: 
        rospy.loginfo("Warning : Serial port will be simulated")
        status.ErrorNr = 0   
    else:
        try:
            ...
            status.ErrorNr = 0
            rospy.loginfo("Serial port connected to machine")
        except SerialException:
            status.ErrorNr = 98
            rospy.loginfo("Error when opening Serial Port")
...
def sendSerialCmd(cmd):
    ...
    rospy.loginfo(" : %s", grbl_out.strip())
...
def sendGCodeFile(file, seq):
    ...
    f = open(file,'r');
    rospy.loginfo("Opening gcode file")
...
def cmdCallback(data):
    ...
    if data.data == 99:
        rospy.loginfo("Starting to home robot")
        status.IsSynced = False
        sendGCodeCmd('G28 X Y Z' + '\n')
        status.IsSynced = True
    elif data.data == 1:
        rospy.loginfo("Send cultivation.g file")
        status.SequenseNr = 1
        sendGCodeFile(path + 'cultivation.g', 1)
    elif data.data == 2:
        rospy.loginfo("Send seed.g file")
        status.SequenseNr = 2
        sendGCodeFile(path + 'seed.g', 2)
    elif data.data == 90:
        rospy.loginfo("Man. pos X")
        sendGCodeCmd('G91\n'+'G0 X10 F1000\n')
        rospy.loginfo("G0 X10 F1000\n")
    elif data.data == 91:
        rospy.loginfo("Man. neg X")
        sendGCodeCmd('G91\n'+'G0 X-10 F1000\n')
        rospy.loginfo("G0 X-10 F1000\n")
    elif data.data == 92:
        rospy.loginfo("Man. pos Y")
        sendGCodeCmd('G91\n'+'G0 Y10 F1000\n')
        rospy.loginfo("G0 Y10 F1000\n")
    elif data.data == 93:
        rospy.loginfo("Man. neg Y")
        sendGCodeCmd('G91\n'+'G0 Y-10 F1000\n')
        rospy.loginfo("G0 Y-10 F1000\n")
    elif data.data == 94:
        rospy.loginfo("Man. pos X pos Y")
        sendGCodeCmd('G91\n'+'G0 X10 Y10 F1000\n')
        rospy.loginfo("G0 X10 Y10 F1000\n")
    elif data.data == 95:
        rospy.loginfo("Man. neg X pos Y")
        sendGCodeCmd('G91\n'+'G0 X-10 Y10 F1000\n')
        rospy.loginfo("G0 X-10 Y10 F1000\n")
    elif data.data == 96:
        rospy.loginfo("Man. pos X neg Y")
        sendGCodeCmd('G91\n'+'G0 X10 Y-10 F1000\n')
        rospy.loginfo("G0 X10 Y-10 F1000\n")
    elif data.data == 97:
        rospy.loginfo("Man. neg X neg Y")
        sendGCodeCmd('G91\n'+'G0 X-10 Y-10 F1000\n')
        rospy.loginfo("G0 X-10 Y-10 F1000\n")
...
def main():
    ...
    rospy.loginfo('')
    rospy.loginfo("Machine is waiting for command..")
    rospy.loginfo('')
